export function SplitText(identifier, vars) {
  ...
  for (var i = 0; i < identifier.length; i++) {
    ...
    if (
      window.jQuery &&
      identifier[i] &&
      (identifier[i] instanceof jQuery || identifier[i].constructor.prototype.jquery)
    ) {
  ...
  if (vars && typeof vars == 'object' && vars !== null) {
    ...
    this.vars.charsClass =
      vars.charsClass && typeof vars.charsClass == 'string' ? vars.charsClass : defaults.charsClass

    //if wordsClass is set then use it
    this.vars.wordsClass =
      vars.wordsClass && typeof vars.wordsClass == 'string' ? vars.wordsClass : defaults.wordsClass

    //if linesClass is set then use it
    this.vars.linesClass =
      vars.linesClass && typeof vars.linesClass == 'string' ? vars.linesClass : defaults.linesClass
  ...
  for (var i = 0; i < this.HTMLobjects.length; i++) {
    ...
    if (this.vars.type.indexOf('words') != -1) {
      function splitWords(parent, st) {
        ...
        var endTag = '</div>'
        parent.innerHTML =
          startTag + parent.innerHTML.replaceAll(' ', endTag + ' ' + startTag) + endTag
